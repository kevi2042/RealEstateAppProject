...
namespace RealEstateApp.Services.Repository
{
    public class MockRepository : IRepository
    {
        ...
        private string _contractFilePath { get; set; }
        ...
        public async Task<LoginResult> GetLoginStorage()
        {
            LoginResult result = new LoginResult();

            result.Succeded = Convert.ToBoolean(await SecureStorage.GetAsync("Login"));
            result.AccessToken = await SecureStorage.GetAsync("Accesstoken");
            result.Refreshtoken = await SecureStorage.GetAsync("RefreshToken");
        ...
        public async Task<LoginResult> LoginAsync(string username, string password)
        {
            ...
            try
            {
                if(!string.IsNullOrWhiteSpace(username) && !string.IsNullOrWhiteSpace(password))
                {
                    ...
                    result.AccessToken = username;
                    result.Refreshtoken = password;

                    await SecureStorage.SetAsync("Login", "true");
                    await SecureStorage.SetAsync("Accesstoken", username);
                    await SecureStorage.SetAsync("RefreshToken", password);
                    return result;
                }
                else
                {
        ...
        private void LoadProperties()
        {
            ...
            _properties = new List<Property>
            {
                new Property
                {
                    ...
                    Aspect = new CompassAspect(){ CurrentAspect = "North" }
                },
                new Property
                {
                    ...
                    NeighbourhoodUrl = "https://en.wikipedia.org/wiki/Collaroy,_New_South_Wales",
                    ContractFilePath = _contractFilePath,
                    Aspect = new CompassAspect(){ CurrentAspect = "East" }
                },
                new Property
                {
                    ...
                    NeighbourhoodUrl = "https://en.wikipedia.org/wiki/Collaroy,_New_South_Wales",
                    ContractFilePath = _contractFilePath,
                    Aspect = new CompassAspect(){ CurrentAspect = "South" } 
                },
                new Property
                {
                ...
                new Property
                {
                    ...
                    NeighbourhoodUrl = "https://en.wikipedia.org/wiki/Collaroy,_New_South_Wales",
                    ContractFilePath = _contractFilePath,
                    Aspect = new CompassAspect(){ CurrentAspect = "West" } 
                }
            };
        }
